            ...
            <select class="form-control" name="schedule_id" required>
              ...
              <? 
                ...
                foreach($schedules as $k => $u1){
                  ...
                  if($chk == 0){
              ?>
                <option value="<? echo $u1->id ?>"><? echo date("m-d-Y", strtotime($u1->start_time))." (".date("h:iA", strtotime($u1->start_time))."-".date("h:iA", strtotime($u1->end_time)).")" ?></option>
              <? }} ?>  
            </select>
          </div>
        </div>
        ...
        <div class="col-md-3" id="employee_id" style="display: none;">
          <div class="form-group">
            ...
            <select class="form-control" name="employee_id" id="eid">
              ...
              <? 
                $users = $this->db->order_by("id", "desc")->get_where("tbl_users", ["role" => "employee", "status" => "Active", "id !="=>$this->session->userdata('user_id')])->result();
    ...
    <div class="table-responsive">
      <table id="example" class="display" style="width:100%;">
        ...
        <tbody>
          ...
          <tr>
            <td><? echo $k+1 ?></td>
            <td><? echo date("m-d-Y", strtotime($sdata->start_time))." (".date("h:iA", strtotime($sdata->start_time))."-".date("h:iA", strtotime($sdata->end_time)).")" ?></td>
            <td><? echo $udata ? $udata->first_name." ".$udata->last_name : '' ?></td>
            <td><? echo $d->request_type ?></td>
            <td><? echo $d->empschedule_id ? date("m-d-Y", strtotime($esdata->start_time))." (".date("h:iA", strtotime($esdata->start_time))."-".date("h:iA", strtotime($esdata->end_time)).")" : '-'; ?></td>
            <? if($this->session->userdata('role') == 'admin'){ ?>
              <td>
